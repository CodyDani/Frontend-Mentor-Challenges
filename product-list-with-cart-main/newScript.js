...
function handleAddToCart(dessert) {
  ...
  updateCartDisplay();
   ...
   //Update Dessert Action (Replace Add Btn with +/-)
...
function updateDessertQuantityUI(dessertName) {
  ...
  // Find the dessert object again (so we can get the image back)
  ...
  img.addEventListener('click', () => showQuantityControl(dessert));
   ...
   //Show Quantity Control on Image Click
...
function resetDessertItemsUI() {
  // For each dessert item in the container
  ...
  dessertItems.forEach(item => {
    ...
    actionDiv.innerHTML = ''; // Clear any quantity control
    ...
    // Recreate the Add to Cart button
    const button = document.createElement("button");
    button.classList.add("add-cart-btn");
    button.innerHTML = `<img src="assets/images/icon-add-to-cart.svg"> Add to Cart`;

    // Append both to the actionDiv
    actionDiv.append(img, button);
